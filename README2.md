...
- Name: `MSAL Node TradeAdviser` (suggested)
...
1. Navigate to http://tradeadviser.org (or whatever port number specified) with the browser of your choice.
...
In this code tradeadviser, the user is immediately signed-in. If you want all users to be logged in before they view anything, then you can use the same process. Add the sign-in code to the default route:
...
```js
    const authCodeUrlParameters = {
        scopes: ["user.read"],
        redirectUri:  process.env.REDIRECT_URI+"/redirect",
    };
```
